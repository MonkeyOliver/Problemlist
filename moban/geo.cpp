...
vector<Line> TangentoutCC(Circle k1, Circle k2)
{
    ...
    if (cmp(k1.r, k2.r) == 0) {
        ...
        return {{k1.o - del * k1.r, k2.o - del * k2.r},
...
Circle getSCircle(vector<Point> A)
{
    ...
    std::mt19937 seed(rd());
    std::shuffle(A.begin(), A.end(), seed);
    Circle ans = {A[0], 0};
...
{
    // 返回点集A组成的凸包 , flag=0 不严格 flag=1 严格
    int n = A.size();
...
class P3 {
   public:
    db x, y, z;
    P3() = default;
    P3 operator+(P3 k1) { return {x + k1.x, y + k1.y, z + k1.z}; }
    P3 operator-(P3 k1) { return {x - k1.x, y - k1.y, z - k1.z}; }
...
{
    return {k1.y * k2.z - k1.z * k2.y, k1.z * k2.x - k1.x * k2.z,
            k1.x * k2.y - k1.y * k2.x};
}
